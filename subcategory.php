...
<div class="wrapper-wide">
  ...
  <div id="container">
    <div class="container">
      ...
      <?php $sql=mysqli_query($con,"select category.categoryName as catname,category.categoryName as catid,subCategory.subcategory as subcatname from subcategory join category on category.id=subcategory.categoryid Where subcategory.id='$cid'");
        while($row=mysqli_fetch_array($sql))
        {
          ?>
        <li><a href="category.php?cid=<?php echo $catid;?>"><?php echo $row['catname'];?></a></li>
        <li><a href="#"><?php echo $row['subcatname'];?></a></li>
      <?php }?>
      </ul>
      <!-- Breadcrumb End-->
      <div class="row">
        ...
        <div id="content" class="col-sm-9">
          <?php $sql=mysqli_query($con,"select subcategory.subcategory,category.categoryName from subcategory JOIN category ON subcategory.categoryid = category.id
 Where subcategory.id='$cid'");
        while($row=mysqli_fetch_array($sql))
        {
          ?>
          <h2 class="title"><?php echo $row['categoryName'];?> - <b><?php echo $row['subcategory'];?></b></h2><?php }?>
          <h3 class="subtitle">Sub Categories</h3>
          <div class="category-list-thumb row">
            <?php $sql=mysqli_query($con,"select * from subcategory Where categoryid='$catid' and id!='$cid'");
              while($row=mysqli_fetch_array($sql))
              {
              ?>
            <div class="col-lg-2 col-md-2 col-sm-2 col-xs-4">
              <a href="subcategory.php?subcategory=<?php echo $row['id'];?>&category=<?php echo $catid;?>">
              ...
              <a href="subcategory.php?subcategory=<?php echo $row['id'];?>&category=<?php echo $catid;?>"><?php echo $row['subcategory'];?>
              </a>
            </div>
            <?php }?>
          </div>
          ...
          <div class="row products-category">
            ...
            $total_pages_sql = "SELECT COUNT(*) FROM products where subCategory='$cid'";
            $result = mysqli_query($con,$total_pages_sql);
            $total_rows = mysqli_fetch_array($result)[0];
            $total_pages = ceil($total_rows / $no_of_records_per_page);

            $ret=mysqli_query($con,"select * from products where subCategory='$cid' LIMIT $offset, $no_of_records_per_page");
    ...
    <?php } ?>
          ...
          <div class="row">
            <div class="col-sm-6 text-left">
              <ul class="pagination"> 
                <li class="active"><a href="subcategory.php?subcategory=<?php echo $cid;?>&category=<?php echo $catid;?>&pageno=1"><span>First</span></a></li>

                <li class="<?php if($pageno <= 1){ echo 'disabled'; } ?>"><a href="<?php if($pageno <= 1){ echo '#'; } else { echo "subcategory.php?subcategory=$cid&category=$catid&pageno=".($pageno - 1); } ?>">Privious</a></li>

                <li class="<?php if($pageno >= $total_pages){ echo 'disabled'; } ?>"><a href="<?php if($pageno >= $total_pages){ echo '#'; } else { echo "subcategory.php?subcategory=$cid&category=$catid&pageno=".($pageno + 1); } ?>">Next</a></li>

                <li class="<?php if($pageno >= $total_pages){ echo 'disabled'; } ?>"><a href="<?php if($pageno >= $total_pages){ echo '#'; } else { echo "subcategory.php?subcategory=$cid&category=$catid&pageno=$total_pages"; } ?>">Last</a></li>
